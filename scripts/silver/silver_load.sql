...
BEGIN
    ...
    --FLATTENED 'Extras' table

    TRUNCATE TABLE silver.card_extras_flattened;
        INSERT INTO silver.card_extras_flattened(card_id,subtype_list,type_list,weaknesses_list,resistances_list,pokedex_list)
        SELECT 
        m.card_id,
        COALESCE(st.subtype_list,'n/a') as subtype_list,
        COALESCE(t.type_list, 'n/a') as type_list,
        COALESCE(w.weaknesses_list, 'n/a') as weaknesses_list,
        COALESCE(r.resistances_list, 'n/a') as resistances_list,
        COALESCE(p.national_pokedex_numbers, 'n/a') as national_pokedex_numbers_list
        FROM silver.card_main_info m
        LEFT JOIN (SELECT card_id,STRING_AGG(subtype,', ') as subtype_list FROM silver.card_subtypes GROUP BY card_id) st
        ON st.card_id = m.card_id 
        LEFT JOIN (SELECT card_id,STRING_AGG(type,', ') as type_list FROM silver.card_types GROUP BY card_id) t
        ON t.card_id = m.card_id 
        LEFT JOIN (SELECT card_id,STRING_AGG(weakness_type,', ')as weaknesses_list FROM silver.card_weaknesses GROUP BY card_id) w
        ON w.card_id = m.card_id 
        LEFT JOIN (SELECT card_id,STRING_AGG(resistance_type,', ') as resistances_list FROM silver.card_resistances GROUP BY card_id) r
        ON r.card_id = m.card_id 
        LEFT JOIN (SELECT card_id,STRING_AGG(national_pokedex_number,', ') as national_pokedex_numbers FROM silver.card_national_pokedex_numbers GROUP BY card_id) p
